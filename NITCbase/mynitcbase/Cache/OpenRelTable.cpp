...
int OpenRelTable::closeRel(int relId)
{
  ...
  AttrCacheEntry *head, *next;

  head = AttrCacheTable::attrCache[relId];
  next = head->next;
  ...
  while (next)
  {
    if (head->dirty == true)
    {
      AttrCatEntry attrCatBuf = head->attrCatEntry;
      Attribute record[ATTRCAT_NO_ATTRS];
      AttrCacheTable::attrCatEntryToRecord(&attrCatBuf, record);
      RecBuffer attrcatBuffer(head->recId.block);
      attrcatBuffer.setRecord(record, head->recId.slot);
    }
    free(head);
    head = next;
    next = next->next;
  ...
  char c = '\0';
  strcpy(OpenRelTable::tableMetaInfo[relId].relName, &c);
